...
O commit `48a7187` publicou um controle de logout acessível na lateral do perfil autenticado. No navegador de produção, a sessão real mostrou o botão `Sair`; ao acioná-lo, a interface confirmou `Sessão encerrada.`, trocou o perfil para `Visitante` e apresentou as ações `Entrar`/`Entrar para salvar`. Junto ao `POST auth.login` e `GET auth.me` com HTTP 200 registrados anteriormente, isso conclui a validação de criação, leitura e remoção do cookie de sessão em HTTPS.

A sanitização de telemetria foi publicada no commit `eb6d52a` e permaneceu na versão posterior `48a7187`. A regressão automatizada cobre a remoção de e-mail, URL de conexão, usuário do banco e IP de mensagens do driver. Como a conexão TiDB já está saudável, não foi induzida uma nova falha de banco em produção apenas para produzir log; a validação usa o teste automatizado e o deployment já ativo para evitar indisponibilidade artificial.
