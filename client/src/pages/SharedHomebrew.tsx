...
import { calculateAttributeModifier, calculateShikigamiReferenceStats, INVOCATION_GRADE_LABELS, INVOCATION_GRADE_RULES, isShikigamiType, SHIKIGAMI_TYPE_LABELS, type InvocationAttribute, type InvocationGrade, type ShikigamiSize } from "@shared/homebrewRules";
...
export function ShikigamiReadCard({ sheet }: { sheet: Record<string, unknown> }) {
  ...
  const optionLabels: Record<string, string> = { concentrarPoder: "Concentrar Poder", fantocheSupremo: "Fantoche Supremo", invocacoesMoveis: "Invocações Móveis", melhoriaResistencia: "Melhoria de Controlador: Resistência", invocacoesEconomicas: "Invocações Econômicas", melhoriaMobilidade: "Melhoria de Controlador: Mobilidade", invocacoesResistentes: "Invocações Resistentes", melhoriaPrecisao: "Melhoria de Controlador: Precisão (CD)", movimentoAlternativo: "Movimento Alternativo", defesaAlternativa: "Defesa Alternativa", bonusPericiaA: "Bônus em Perícia A", bonusPericiaB: "Bônus em Perícia B", bonusPericiaC: "Bônus em Perícia C", tamanho: "Tamanho", defensor: "Defensor", robustez: "Robustez", movel: "Móvel", perito: "Perito" };
  ...
  const healthPercentage = stats?.health ? Math.min(100, Math.round((currentHealth * 100) / stats.health)) : 0;
  ...
  const skillDefinitions: ReadonlyArray<readonly [string, string, InvocationAttribute]> = [["feiticaria", "Feitiçaria", "inteligencia"], ["investigacao", "Investigação", "inteligencia"], ["historia", "História", "inteligencia"], ["medicina", "Medicina", "sabedoria"], ["religiao", "Religião", "inteligencia"], ["ocultismo", "Ocultismo", "sabedoria"], ["prestidigitacao", "Prestidigitação", "destreza"], ["percepcao", "Percepção", "sabedoria"], ["intuicao", "Intuição", "sabedoria"], ["furtividade", "Furtividade", "destreza"], ["oficio", "Ofício", "inteligencia"], ["reflexos", "Reflexos", "destreza"], ["fortitude", "Fortitude", "constituicao"], ["vontade", "Vontade", "sabedoria"], ["astucia", "Astúcia", "inteligencia"], ["integridade", "Integridade", "constituicao"]];
  const skillTotals = stats ? skillDefinitions.map(([key, label, attribute]) => {
    const state = asRecord(skills[key]) ?? {};
    const masteryEnabled = Boolean(state.mastery ?? state.trained);
    const specialtyEnabled = Boolean(state.specialty);
    const traitBonus = (traits.bonusPericiaA && (sheet.bonusSkillA ?? sheet.bonusSkill) === key ? stats.skillBonusPerSelection : 0) + (traits.bonusPericiaB && sheet.bonusSkillB === key ? stats.skillBonusPerSelection : 0) + (traits.bonusPericiaC && sheet.bonusSkillC === key ? stats.skillBonusPerSelection : 0);
    const total = Number(state.otherBonus ?? state.manualBonus ?? 0) + Math.floor(userLevel / 2) + calculateAttributeModifier(Math.max(stats.attributeBase, Number(attributes[attribute] ?? stats.attributeBase))) + (specialtyEnabled ? stats.skillMasteryBonus : masteryEnabled ? stats.skillTypeBonus : 0) + traitBonus;
    return { key, label, total };
  }) : [];

  return <section className="mt-7 rounded-2xl border border-rose-300/12 bg-rose-500/[0.035] p-5"><div className="flex items-center gap-2"><Flame size={16} className="text-rose-300" /><div><p className="text-sm font-semibold text-stone-100">{String(sheet.name || "Shikigami")}</p><p className="mt-0.5 text-[10px] uppercase tracking-[0.14em] text-stone-500">{SHIKIGAMI_TYPE_LABELS[type]} · {grade ? INVOCATION_GRADE_LABELS[grade] : "grau não definido"} · nível {userLevel}</p></div></div>{statEntries.length > 0 && <><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">{statEntries.map(([label, value]) => <div key={String(label)} className="rounded-lg border border-white/8 bg-black/10 p-2 text-center"><p className="text-sm font-semibold text-rose-100">{String(value)}</p><p className="mt-1 text-[9px] uppercase text-stone-500">{String(label)}</p></div>)}</div><div className="mt-3" aria-label={`Estado da invocação: ${healthPercentage}%`} role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={healthPercentage}><div className="h-1.5 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-rose-400 transition-[width] duration-200" style={{ width: `${healthPercentage}%` }} /></div><p className="mt-1 text-right text-[9px] text-stone-500">{healthPercentage}% de vida atual</p></div></>}<div className="mt-4"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-stone-500">Atributos</p><div className="mt-2 flex flex-wrap gap-2">{Object.entries(attributeLabels).map(([attribute, label]) => { const value = Math.max(stats?.attributeBase ?? 8, Number(attributes[attribute] ?? stats?.attributeBase ?? 8)); return <span key={attribute} className="rounded-md border border-white/8 bg-black/10 px-2 py-1 text-[10px] text-stone-300">{label}: <strong className="text-rose-100">{value}</strong> ({calculateAttributeModifier(value) >= 0 ? "+" : ""}{calculateAttributeModifier(value)})</span>; })}</div></div>{skillTotals.length > 0 && <div className="mt-4"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-stone-500">Perícias calculadas</p><div className="mt-2 grid gap-1 sm:grid-cols-2 lg:grid-cols-4">{skillTotals.map(skill => <div key={skill.key} className="flex items-center justify-between rounded-md border border-white/8 bg-black/10 px-2 py-1.5 text-[10px]"><span className="text-stone-400">{skill.label}</span><strong className="text-rose-100">{skill.total >= 0 ? "+" : ""}{skill.total}</strong></div>)}</div></div>}{selectedOptions.length > 0 && <div className="mt-4"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-stone-500">Opções escolhidas</p><div className="mt-2 flex flex-wrap gap-1.5">{selectedOptions.map(option => <span key={option} className="rounded-md border border-fuchsia-300/15 bg-fuchsia-500/[.08] px-2 py-1 text-[10px] text-fuchsia-100">{option}</span>)}</div></div>}{stats && Boolean(traits.tamanho) && <p className="mt-3 text-[10px] text-stone-500">Tamanho: ataques {stats.sizeAttackModifier >= 0 ? "+" : ""}{stats.sizeAttackModifier}; resistências {stats.sizeResistanceModifier >= 0 ? "+" : ""}{stats.sizeResistanceModifier}.</p>}<ReadEntries title="Ações" entries={byKind("acao")} /><ReadEntries title="Características adicionais" entries={byKind("caracteristica")} />{typeof sheet.notes === "string" && sheet.notes.trim() && <div className="mt-4"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-stone-500">Anotações manuais</p><p className="mt-2 whitespace-pre-wrap text-[11px] leading-relaxed text-stone-400">{sheet.notes}</p></div>}</section>;
}
