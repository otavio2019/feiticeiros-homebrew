...
function StructuredElementsPanel({ homebrewId, moduleId, moduleType }: { homebrewId: number | null; moduleId?: number; moduleType: HomebrewModuleType }) {
  ...
  const uploadMutation = trpc.homebrew.uploadImage.useMutation({ onSuccess: image => { setElementImageUrl(image.url); toast.success("Imagem carregada para o elemento."); }, onError: error => toast.error(error.message) });
  ...
  return <section className="rounded-2xl border border-rose-300/12 bg-rose-500/[0.035] p-5">
    ...
    <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_1fr_auto_auto]"><Input value={name} onChange={event => setName(event.target.value)} placeholder={structuredEditorGuidance[moduleType].element} className="h-9 bg-black/15 text-xs" /><Input value={description} onChange={event => setDescription(event.target.value)} placeholder="Descrição e funcionamento" className="h-9 bg-black/15 text-xs" /><div className="flex gap-1"><Input value={elementImageUrl} onChange={event => setElementImageUrl(event.target.value)} placeholder="URL da imagem (opcional)" className="h-9 bg-black/15 text-xs" /><label className="flex h-9 cursor-pointer items-center justify-center rounded-md border border-white/8 bg-white/5 px-3 text-stone-400 hover:bg-white/10"><ImagePlus size={14} /><input type="file" className="hidden" accept="image/*" onChange={e => { const file = e.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { const base64 = String(reader.result).split(",")[1] ?? ""; if (base64) uploadMutation.mutate({ homebrewId, fileName: file.name, contentType: file.type as "image/jpeg" | "image/png" | "image/webp", base64, moduleId, altText: `Imagem de ${name.trim() || structuredEditorGuidance[moduleType].element}` }); }; reader.readAsDataURL(file); }} /></label></div><Button disabled={!name.trim() || !description.trim() || createMutation.isPending} onClick={() => createMutation.mutate({ homebrewId, moduleId, type: typeMap[moduleType], name: name.trim(), description: description.trim(), isManual: true })} className="h-9 gap-1 bg-rose-500 px-3 text-xs text-white hover:bg-rose-400"><Plus size={13} /> Adicionar</Button></div>
