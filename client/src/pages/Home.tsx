...
import { StructuredElementChildren } from "@/components/StructuredElementChildren";
import { StructuredElementSelector } from "@/components/StructuredElementSelector";
import { buildHomebrewValidation, HOME_BREW_MODULE_LABELS, HOME_BREW_MODULES, SPELL_COST_BY_LEVEL, STRUCTURED_DAMAGE_TYPES, type HomebrewModuleType } from "@shared/homebrewRules";
import {
  Archive,
  ArrowDown,
  ArrowRight,
...
type ChildCollectionKey = "requirements" | "attributeBonuses" | "effects" | "costs" | "damageProfiles" | "ranges" | "conditions" | "vowExchanges" | "evolutions";
...
const childEditorFields: Record<ChildCollectionKey, ChildEditorField[]> = {
  ...
  vowExchanges: [{ key: "kind", label: "Tipo (gain ou loss)" }, { key: "description", label: "Descrição" }, { key: "valueNumber", label: "Valor numérico", type: "number" }],
...
export function StructuredElementsPanel({ homebrewId, moduleId, moduleType }: { homebrewId: number | null; moduleId?: number; moduleType: HomebrewModuleType }) {
  ...
  const [vowExchangeKind, setVowExchangeKind] = useState<"gain" | "loss">("gain");
  const [vowExchangeDescription, setVowExchangeDescription] = useState("");
  const [vowExchangeValue, setVowExchangeValue] = useState("");
  ...
  const saveExtendedMutation = trpc.homebrew.structuredSaveExtendedMechanics.useMutation({ onSuccess: () => { setCostResource(""); setCostDetails(""); setDamageDice(""); setDamageType(""); setDamageDetails(""); setConditionName(""); setConditionEffect(""); setEvolutionName(""); setEvolutionDescription(""); setVowExchangeDescription(""); setVowExchangeValue(""); mechanicsQuery.refetch(); toast.success("Custos, dano e progressões salvos."); }, onError: error => toast.error(error.message) });
  ...
  const replaceVowExchanges = (next: Array<{ kind: "gain" | "loss"; description: string; valueNumber?: number | null }>) => { if (!activeElementId) return; saveExtendedMutation.mutate({ homebrewId: homebrewId ?? 0, elementId: activeElementId, vowExchanges: next }); };
  const editExtendedItem = (key: "costs" | "damageProfiles" | "ranges" | "conditions" | "vowExchanges" | "evolutions", index: number) => { const current = mechanicsQuery.data; if (!current) return; const item: any = (current[key] ?? [])[index]; setChildEditor({ key, index, values: Object.fromEntries(childEditorFields[key].map(field => [field.key, item?.[field.key] == null ? "" : String(item[field.key])])) }); };
  const saveChildEditor = () => { if (!childEditor || !mechanicsQuery.data) return; const current: any = mechanicsQuery.data; const item: any = (current[childEditor.key] ?? [])[childEditor.index]; if (!item) return; const updated: any = { ...item }; for (const field of childEditorFields[childEditor.key]) { const raw = childEditor.values[field.key] ?? ""; updated[field.key] = field.control === "checkbox" ? raw === "true" : field.type === "number" ? (raw.trim() === "" ? null : Number(raw)) : raw.trim(); } const next = [...(current[childEditor.key] ?? [])] as any[]; next[childEditor.index] = updated; if (["requirements", "attributeBonuses", "effects"].includes(childEditor.key)) replaceCoreCollection(childEditor.key as "requirements" | "attributeBonuses" | "effects", next); else if (childEditor.key === "vowExchanges") replaceVowExchanges(next.map(exchange => ({ kind: exchange.kind === "loss" ? "loss" : "gain", description: String(exchange.description ?? ""), valueNumber: exchange.valueNumber == null ? null : Number(exchange.valueNumber) }))); else replaceExtendedCollection(childEditor.key as "costs" | "damageProfiles" | "ranges" | "conditions" | "evolutions", next); setChildEditor(null); };
  ...
  return <section className="rounded-2xl border border-rose-300/12 bg-rose-500/[0.035] p-5">
    ...
    <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_1fr_130px_auto_auto]"><Input value={name} onChange={event => setName(event.target.value)} placeholder={structuredEditorGuidance[moduleType].element} className="h-9 bg-black/15 text-xs" /><Input value={description} onChange={event => setDescription(event.target.value)} placeholder="Descrição e funcionamento" className="h-9 bg-black/15 text-xs" /><select aria-label="Fonte da regra do elemento" value={ruleSource} onChange={event => setRuleSource(event.target.value as "official" | "homebrew" | "manual")} className="h-9 rounded-md border border-white/8 bg-[#201b29] px-2 text-xs text-stone-300"><option value="homebrew">Homebrew</option><option value="official">Oficial</option><option value="manual">Manual</option></select><div className="flex gap-1"><Input value={elementImageUrl} onChange={event => setElementImageUrl(event.target.value)} placeholder={pendingImageFile ? `Arquivo: ${pendingImageFile.name}` : "URL da imagem (opcional)"} className="h-9 bg-black/15 text-xs" /><label className="flex h-9 cursor-pointer items-center justify-center rounded-md border border-white/8 bg-white/5 px-3 text-stone-400 hover:bg-white/10"><ImagePlus size={14} /><input type="file" className="hidden" accept="image/*" onChange={e => { const file = e.target.files?.[0]; if (file) setPendingImageFile(file); }} /></label></div><Button disabled={!name.trim() || !description.trim() || createMutation.isPending} onClick={() => createMutation.mutate({ homebrewId, moduleId, type: typeMap[moduleType], name: name.trim(), description: description.trim(), ruleSource, isManual: ruleSource === "manual" })} className="h-9 gap-1 bg-rose-500 px-3 text-xs text-white hover:bg-rose-400"><Plus size={13} /> Adicionar</Button></div>
    ...
    {activeElementId && moduleType === "votos" && <div className="mt-3 rounded-xl border border-fuchsia-300/15 bg-fuchsia-500/[0.035] p-4"><p className="text-[10px] font-semibold uppercase tracking-wide text-fuchsia-200">Ganhos e perdas do Voto</p><p className="mt-1 text-[11px] text-stone-500">Registre separadamente a contrapartida e o benefício; valores numéricos são opcionais e não recebem fórmulas automáticas.</p><div className="mt-3 grid gap-2 sm:grid-cols-[130px_1fr_110px_auto]"><select aria-label="Tipo de troca do Voto" value={vowExchangeKind} onChange={event => setVowExchangeKind(event.target.value as "gain" | "loss")} className="h-8 rounded-md border border-white/8 bg-[#201b29] px-2 text-xs text-stone-300"><option value="gain">Ganho</option><option value="loss">Perda</option></select><Input value={vowExchangeDescription} onChange={event => setVowExchangeDescription(event.target.value)} placeholder="Descrição do ganho ou da perda" className="h-8 bg-black/15 text-xs" /><Input type="number" value={vowExchangeValue} onChange={event => setVowExchangeValue(event.target.value)} placeholder="Valor opcional" className="h-8 bg-black/15 text-xs" /><Button size="sm" disabled={!vowExchangeDescription.trim() || saveExtendedMutation.isPending} onClick={() => replaceVowExchanges([...(mechanicsQuery.data?.vowExchanges ?? []).map(item => ({ kind: item.kind, description: item.description, valueNumber: item.valueNumber })), { kind: vowExchangeKind, description: vowExchangeDescription.trim(), valueNumber: vowExchangeValue.trim() === "" ? null : Number(vowExchangeValue) }])}>Adicionar</Button></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><StructuredChildList title="Ganhos" items={(mechanicsQuery.data?.vowExchanges ?? []).filter(item => item.kind === "gain")} label={(item: any) => `${item.description}${item.valueNumber == null ? "" : ` (${item.valueNumber})`}`} onEdit={index => { const target = (mechanicsQuery.data?.vowExchanges ?? []).map((item, itemIndex) => ({ item, itemIndex })).filter(entry => entry.item.kind === "gain")[index]; if (target) editExtendedItem("vowExchanges", target.itemIndex); }} onRemove={index => replaceVowExchanges((mechanicsQuery.data?.vowExchanges ?? []).filter((item, itemIndex) => !(item.kind === "gain" && (mechanicsQuery.data?.vowExchanges ?? []).filter((candidate, candidateIndex) => candidateIndex < itemIndex && candidate.kind === "gain").length === index)).map(item => ({ kind: item.kind, description: item.description, valueNumber: item.valueNumber })))} onMove={(index, direction) => { const gains = (mechanicsQuery.data?.vowExchanges ?? []).filter(item => item.kind === "gain"); const others = (mechanicsQuery.data?.vowExchanges ?? []).filter(item => item.kind !== "gain"); replaceVowExchanges([...moveItem(gains, index, direction), ...others].map(item => ({ kind: item.kind, description: item.description, valueNumber: item.valueNumber }))); }} /><StructuredChildList title="Perdas" items={(mechanicsQuery.data?.vowExchanges ?? []).filter(item => item.kind === "loss")} label={(item: any) => `${item.description}${item.valueNumber == null ? "" : ` (${item.valueNumber})`}`} onEdit={index => { const target = (mechanicsQuery.data?.vowExchanges ?? []).map((item, itemIndex) => ({ item, itemIndex })).filter(entry => entry.item.kind === "loss")[index]; if (target) editExtendedItem("vowExchanges", target.itemIndex); }} onRemove={index => replaceVowExchanges((mechanicsQuery.data?.vowExchanges ?? []).filter((item, itemIndex) => !(item.kind === "loss" && (mechanicsQuery.data?.vowExchanges ?? []).filter((candidate, candidateIndex) => candidateIndex < itemIndex && candidate.kind === "loss").length === index)).map(item => ({ kind: item.kind, description: item.description, valueNumber: item.valueNumber })))} onMove={(index, direction) => { const losses = (mechanicsQuery.data?.vowExchanges ?? []).filter(item => item.kind === "loss"); const others = (mechanicsQuery.data?.vowExchanges ?? []).filter(item => item.kind !== "loss"); replaceVowExchanges([...others, ...moveItem(losses, index, direction)].map(item => ({ kind: item.kind, description: item.description, valueNumber: item.valueNumber }))); }} /></div></div>}
    {activeElementId && moduleType === "origem" && <div className="mt-3 space-y-3"><StructuredElementChildren homebrewId={homebrewId} moduleId={moduleId} moduleType={moduleType} parentElementId={activeElementId} allowedKinds={["caracteristica"]} sectionTitle="Características" sectionDescription="Adicione, edite, remova e reordene as características da Origem. Cada uma possui efeitos próprios quando aplicável." /><StructuredElementChildren homebrewId={homebrewId} moduleId={moduleId} moduleType={moduleType} parentElementId={activeElementId} allowedKinds={["talento"]} sectionTitle="Talentos" sectionDescription="Cada Talento pode receber requisitos, efeito, bônus e progressões próprias." /><StructuredElementChildren homebrewId={homebrewId} moduleId={moduleId} moduleType={moduleType} parentElementId={activeElementId} allowedKinds={["penalidade"]} sectionTitle="Penalidades" sectionDescription="Registre as limitações da Origem em itens separados e com efeitos estruturados." /><StructuredElementChildren homebrewId={homebrewId} moduleId={moduleId} moduleType={moduleType} parentElementId={activeElementId} allowedKinds={["evolucao"]} sectionTitle="Evoluções" sectionDescription="Cada Evolução possui requisitos, bônus, penalidades e os Talentos ou Características que ela libera." /></div>}
    {activeElementId && moduleType === "armas" && <div className="mt-3 space-y-3"><StructuredElementChildren homebrewId={homebrewId} moduleId={moduleId} moduleType={moduleType} parentElementId={activeElementId} allowedKinds={["caracteristica"]} sectionTitle="Características da Arma" sectionDescription="Adicione traços próprios da Arma e seus efeitos mecânicos quando aplicável." /><StructuredElementChildren homebrewId={homebrewId} moduleId={moduleId} moduleType={moduleType} parentElementId={activeElementId} allowedKinds={["propriedade"]} sectionTitle="Propriedades da Arma" sectionDescription="Adicione propriedades em registros estruturados, sem depender de um campo de texto único." /></div>}
                ...
                <div className="min-w-0 flex-1 overflow-y-auto bg-[#0e0d15]/35 px-7 py-7 xl:px-10">
                  <div className="mx-auto max-w-3xl">
                    ...
                    <div className="mt-7 space-y-4">
                      ...
                      {["aptidoes", "especializacoes"].includes(activeModule) && <SpecificModuleConfiguration module={activeModule} manualMode={manualMode} onManualMode={setManualMode} data={draftData} onData={setDraftData} />}
